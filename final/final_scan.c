...
void scan(int angle, cyBot_Scan* getScan) {
    servo_move(angle);
    timer_waitMillis(70); // enough time for servo to move

    ping_trigger();
    timer_waitMicros(600);  // much faster than millis

    if(update_flag >= 2) {
        unsigned long time_diff = last_time - current_time;
        float ms;
        getScan->sound_dist = ping_getDistance(time_diff, &ms);
        update_flag = 0;
    }

    getScan->IR_raw_val = adc_read();  // quick ADC read
}
...
void scanFullForObjects(void) {
    ...
    if (object_count == 0) {
        ...
        char msg[100];  // FIXED: declare msg
...
int checkBumpOrCliffDuringMove(oi_t* sensor_data) {
    oi_update(sensor_data);  // Always update first
    ...
    // Only detect tape if sensors are above 2500
    if (cliffLeft > 2500 || cliffFrontLeft > 2500 ||
        cliffFrontRight > 2500 || cliffRight > 2500) {
